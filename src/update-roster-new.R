scrape_old_roster <- function(team, )

build_rosters <-
  function(season = nflreadr:::most_recent_season(roster = TRUE)) {
    # if NGS rosters are not available, default to sleeper API
    if (season >= 2016) {
      ...
      rosters <-
        purrr::map_dfr(teams$team_id, ngsscrapR::scrape_current_roster, season)
    ...
    # comb <-
    #   purrr::map_dfr(1999:latest_season, ~ readRDS(glue::glue("data/seasons/roster_{.x}.rds"))) |>
    #   dplyr::select(-tidyselect::any_of(c("update_dt"))) |>
    #   dplyr::left_join(
    #     readRDS("src/pfr_gsis_map.rds"),
    #     by = "gsis_id",
    #     suffix = c("", "_joined")
    #   ) |>
...
# run annually to update older seasons
